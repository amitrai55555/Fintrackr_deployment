    ...
    public ResponseEntity<?> quickUpdateGoal(@PathVariable Long id,
        ...
        try {
            ...
            LocalDate targetDate = request.containsKey("targetDate")
                    ...
                    : null;
            BigDecimal monthlyContribution = request.containsKey("monthlyContribution")
                    ? new BigDecimal(request.get("monthlyContribution").toString())
                    : null;

            Goal updatedGoal = goalService.quickUpdateGoal(id, addAmount, targetAmount, targetDate,
                    monthlyContribution, userPrincipal.getId());
            return ResponseEntity.ok(updatedGoal);
        } catch (Exception e) {
            Map<String, String> error = new HashMap<>();
            error.put("error", e.getMessage());
