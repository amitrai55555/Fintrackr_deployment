...
public class DashboardService {
        ...
        public List<TransactionDTO> getTransactionsByDateRange(Long userId, LocalDate from, LocalDate to) {
                List<TransactionDTO> transactions = new java.util.ArrayList<>();

                incomeService.getIncomesByUserIdAndDateRange(userId, from, to)
                                .stream()
                                .map(this::mapIncomeToTransactionDto)
                                .forEach(transactions::add);

                expenseService.getExpensesByUserIdAndDateRange(userId, from, to)
                                .stream()
                                .map(this::mapExpenseToTransactionDto)
                                .forEach(transactions::add);
        ...
        private TransactionDTO mapIncomeToTransactionDto(Income income) {
        ...
        private TransactionDTO mapExpenseToTransactionDto(Expense expense) {
                return new TransactionDTO(
                                expense.getId(),
