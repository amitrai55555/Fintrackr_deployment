...
public class GoalService {
    ...
    public Goal quickUpdateGoal(Long goalId, BigDecimal addAmount, BigDecimal newTargetAmount, LocalDate newTargetDate,
            BigDecimal newMonthlyContribution, Long userId) {
        Goal goal = getGoalById(goalId, userId)
                .orElseThrow(() -> new RuntimeException("Goal not found"));
        ...
        // Update monthly contribution if provided (allow 0 to clear/stop contributions)
        if (newMonthlyContribution != null && newMonthlyContribution.compareTo(BigDecimal.ZERO) >= 0) {
            goal.setMonthlyContribution(newMonthlyContribution);
        }
